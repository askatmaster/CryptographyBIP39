...
public class BIP39
{
    ...
    public BIP39(int entropySize = cMinimumEntropyBits, string passphrase = cEmptyString, Language language = Language.English)
    {
        ...
        if (entropySize % cEntropyMultiple != 0 || entropySize is < cMinimumEntropyBits or > cMaximumEntropyBits)
            throw new Exception("entropy size must be a multiple of "
                              + cEntropyMultiple
                              + " (divisible by "
                              + cEntropyMultiple
                              + " with no remainder) and must be greater than "
                              + (cMinimumEntropyBits - 1)
                              + " and less than "
                              + (cMaximumEntropyBits + 1));
    ...
    public BIP39(byte[] entropyBytes, string passphrase = cEmptyString, Language language = Language.English)
    {
        ...
        if (entropyBytes.Length * cBitsInByte % cEntropyMultiple != 0 || entropyBytes.Length * cBitsInByte < cMinimumEntropyBits)
            throw new Exception("entropy bytes must be a multiple of "
                              + cEntropyMultiple / cBitsInByte
                              + " (divisible by "
                              + cEntropyMultiple / cBitsInByte
                              + " with no remainder) and must be greater than "
                              + (cMinimumEntropyBits / cBitsInByte - 1)
                              + " bytes and less than "
                              + (cMaximumEntropyBits / cBitsInByte + 1)
                              + " bytes");
    ...
    public BIP39(string mnemonicSentence, string passphrase = cEmptyString, Language language = Language.Unknown)
    {
        ...
        _passphraseBytes = Encoding.UTF8.GetBytes(Utilities.NormaliseStringNfkd(passphrase));
        var words = _mnemonicSentence.Split(new [] { ' ' });
        ...
        if (words.Length < 12 || words.Length % 3 != 0)
            throw new
                Exception("Mnemonic sentence must be at least 12 words and it will increase by 3 words for each increment in entropy. "
                        + "Please ensure your sentence is at leas 12 words and has no remainder when word count is divided by 3");
    ...
    {
        var entropyBytes = await Utilities.GetRandomBytesAsync(entropySize / cBitsInByte);
    ...
    {
        var eng = new Wordlists.English();
        var russian = new Wordlists.Russian();
        var jp = new Wordlists.Japanese();
        var es = new Wordlists.Spanish();
        var fr = new Wordlists.French();
        var cnS = new Wordlists.ChineseSimplified();
        var cnT = new Wordlists.ChineseTraditional();

        var languageCount = new List<int>(new [] { 0, 0, 0, 0, 0, 0 });

        foreach(var s in words)
        {
            if(eng.WordExists(s, out var _))

                //english is at 0
                languageCount[0]++;

            if (jp.WordExists(s, out var _))

                //japanese is at 1
                languageCount[1]++;

            if (es.WordExists(s, out var _))

                //spanish is at 2
                languageCount[2]++;

            if (cnS.WordExists(s, out var _))

                //chinese simplified is at 3
                languageCount[3]++;

            if (cnT.WordExists(s, out var _) && ! cnS.WordExists(s, out var _))

                //chinese traditional is at 4
                languageCount[4]++;

            if (fr.WordExists(s, out var _))

                //french is at 5
                languageCount[5]++;

            if(russian.WordExists(s, out var _))
        ...
        return languageCount.IndexOf(languageCount.Max()) switch
        {
            ...
            3 => languageCount[4] > 0 ?
                ...
                Language.ChineseTraditional : Language.ChineseSimplified,
            4 => Language.ChineseTraditional,
            _ => languageCount.IndexOf(languageCount.Max()) == 5 ? Language.French : Language.Unknown
        };
    }
    ...
    public static byte[] GetSeedBytes(string mnemonicSentence, string passphrase = cEmptyString)
    {
        ...
        var salt = Utilities.MergeByteArrays(Encoding.UTF8.GetBytes(cSaltHeader), Encoding.UTF8.GetBytes(Utilities.NormaliseStringNfkd(passphrase)));

        return RFC2898_PBKDF2_HMACSHA512.PBKDF2(Encoding.UTF8.GetBytes(mnemonicSentence), salt);
    }
    ...
    public static string GetSeedBytesHexString(string mnemonicSentence, string passphrase = cEmptyString)
    {
        ...
        return Utilities.BytesToHexString(RFC2898_PBKDF2_HMACSHA512.PBKDF2(Encoding.UTF8.GetBytes(mnemonicSentence), salt));
    }
    ...
    private void pInit(string passphrase, Language language)
    {
        _passphraseBytes = Encoding.UTF8.GetBytes(Utilities.NormaliseStringNfkd(passphrase));
        _language = language;
        var allChecksumBytes = Utilities.Sha256Digest(_entropyBytes, 0, _entropyBytes.Length); //sha256 the entropy bytes to get all the checksum bits
        ...
        var numberOfChecksumBits
            = _entropyBytes.Length * cBitsInByte / cEntropyMultiple; //number of bits to take from the checksum bits, varies on entropy size as per spec
        var entropyConcatChecksumBits = new BitArray(_entropyBytes.Length * cBitsInByte + numberOfChecksumBits);
        ...
        var checksumBytesToUse = new List<byte>();

        var byteCount = Math.Ceiling((double)numberOfChecksumBits / cBitsInByte);

        for (var i = 0; i < byteCount ; i++)
            checksumBytesToUse.Add(allChecksumBytes[i]);

        var ba = new BitArray(checksumBytesToUse.ToArray());

        //add checksum bits
        for(var i = 0; i < numberOfChecksumBits; i++)
        {
        ...
        foreach (bool i in entropyConcatChecksumBits)
            sb.Append(i ? "1" : "0");
    ...
    private string pGetMnemonicSentence()
    {
        ...
        if (_wordIndexList.Contains(-1))
            throw new
                Exception(
                    "the wordlist index contains -1 which means words were used in the mnemonic sentence that cannot be found in the wordlist and the index to sentence feature cannot be used. Perhaps a different language wordlist is needed?");

        var mSentence = cEmptyString;

        Wordlists.Wordlist wordlist = _language switch
        {
            Language.English => new Wordlists.English(),
            Language.Russian => new Wordlists.Russian(),
            Language.Japanese => new Wordlists.Japanese(),
            Language.Spanish => new Wordlists.Spanish(),
            Language.ChineseSimplified => new Wordlists.ChineseSimplified(),
            Language.ChineseTraditional => new Wordlists.ChineseTraditional(),
            Language.French => new Wordlists.French(),
            _ => new Wordlists.English()
        };

        for(var i = 0; i < _wordIndexList.Count; i++)
        {
    ...
    {
        var wordIndexList = new List<int>();
        ...
        for(var i = 0; i < entropyConcatChecksumBits.Length; i += cBitGroupSize)
        {
            var toInt = new BitArray(cBitGroupSize);
            for (var i2 = 0; i2 < cBitGroupSize && i < entropyConcatChecksumBits.Length; i2++)
    ...
    private List<int> pRebuildWordIndexes(string[] wordsInMnemonicSentence)
    {
        ...
        switch (_language)
        {
            ...
            case Language.Russian:
                wordlist = new Wordlists.Russian();
                langName = "Russian";
        ...
        foreach(var s in wordsInMnemonicSentence)
        {
            if(!wordlist.WordExists(s, out var idx))
    ...
    private byte[] pProcessIntToBitsThenBytes(List<int> wordIndex)
    {
        ...
        if (wordIndex.Contains(-1))
            throw new
                Exception("the wordlist index contains -1 which means words were used in the mnemonic sentence "
                        + "that cannot be found in the wordlist and so the -1 will stuff up our entropy bits "
                        + "and we cannot rebuild the entropy from index containing -1. Perhaps a different language wordlist is needed?");

        var bits = new BitArray(wordIndex.Count * cBitGroupSize);
        ...
        for(var i = 0; i < wordIndex.Count; i++)
        {
            var wordindex = (double)wordIndex[i];

            //slide down our 11 bits doin mod 2 to determin true or false for each bit
            for (var biti = 0; biti < 11; biti++)
            {
                bits[bitIndex] = false;
            ...
            var temp = bits.Get(bitIndex - cBitGroupSize);
        ...
        var length = bits.Length - bits.Length / (cEntropyMultiple + 1);

        if (length % 8 != 0)
            throw new Exception("Entropy bits less checksum need to be cleanly divisible by " + cBitsInByte);

        var entropy = new byte[length / cBitsInByte];
        var checksum = new BitArray(bits.Length / (cEntropyMultiple + 1));
        var checksumActual = new BitArray(bits.Length / (cEntropyMultiple + 1));

        var index = 0;

        //get entropy bytes
        for (var byteIndex = 0; byteIndex < entropy.Length; byteIndex++)
        {
            for(var i = 0; i < cBitsInByte; i++)
            {
                var bitIdx = index % cBitsInByte;
        ...
        var allChecksumBits
            = new BitArray(Utilities.SwapEndianBytes(Utilities.Sha256Digest(Utilities.SwapEndianBytes(entropy),
                                                                            0,
                                                                            entropy.Length))); //sha256 the entropy bytes to get all the checksum bits

        for(var i = 0; i < checksumActual.Length; i++)
        ...
        if (checksumActual.Xor(checksum).Cast<bool>().Any(b => b))
    ...
    /// </summary>
    public byte[] EntropyBytes => Utilities.SwapEndianBytes(_entropyBytes);
    ...
    {
        set => _passphraseBytes = Encoding.UTF8.GetBytes(Utilities.NormaliseStringNfkd(value));
    }
    ...
    public string MnemonicSentence
    {
        get
        {
            var outputMnemonic = _mnemonicSentence;

            if(_language.Equals(Language.Japanese))
            {
                char.TryParse(cJPSpaceString, out var japSpace);
    ...
    public byte[] SeedBytes
    {
        get
        {
            ...
            var salt = Utilities.MergeByteArrays(Encoding.UTF8.GetBytes(cSaltHeader), _passphraseBytes);

            return RFC2898_PBKDF2_HMACSHA512.PBKDF2(Encoding.UTF8.GetBytes(Utilities.NormaliseStringNfkd(MnemonicSentence)), salt);
        }
    }
    ...
    /// </summary>
    public string SeedBytesHexString => Utilities.BytesToHexString(SeedBytes);
    ...
    public int WordCountFromEntropy
    {
        get
        {
            var entropyBits = _entropyBytes.Length * cBitsInByte;
